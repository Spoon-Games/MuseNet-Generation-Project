...
namespace MuseL
{
    public class MuseReciever : IDisposable
    {
        ...
        private string startMelodieName;
        ...
        public MuseReciever(MidiFile startMelodie, string genre, Instruments instruments, int temperature, int turnication, float length,string startMelodieName = "")
        {
            ...
            this.startMelodieName = startMelodieName.Replace("F-"+genre,"");
        }

        public string GetName()
        {
            return "F-"+genre+startMelodieName;
        }
        ...
        public async Task Recieve(CancellationToken cancellation)
        {
            int durration;
            string[] encoding;
            HttpClient client = MuseNetworkRequest.GetClient();
            ...
            while(durration < totalDurration)
            {
                ...
                if (encoding == null)
                    break; 
                else if(encoding.Length == 1 && Equals(encoding[0], "noresponse"))
                {
                    IsNoResponse = true;
                    break;
                }
        ...
        public void Dispose()
        {
            Debug.Log("Dispose MuseReciever");
            cts?.Cancel();
            cts?.Dispose();
        }
    }
